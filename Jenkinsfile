...
pipeline {
    agent any
    environment {
        DOCKER_IMAGE = 't0ky0le/sprint-service:1.0' // Nombre de la imagen en Docker Hub
    }
    stages {
        stage('Checkout') {
            steps {
                echo 'Clonando el repositorio...'
                git branch: 'main', url: 'https://github.com/TokyoF/sprint-service-taskflow.git' // Cambia con tu URL
            }
        }
        stage('Build') {
            steps {
        ...
        stage('Docker Compose Down') {
            steps {
                ...
                bat 'docker rm -f sprint-service || true' // Elimina el contenedor si existe
